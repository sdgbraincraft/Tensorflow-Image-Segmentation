...
extension ViewController {
  ...
  func runSegmentation(_ image: UIImage) {
    ...
    let willCenterCrop = true//cropSwitch.isOn
...
extension ViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
  ...
  func imagePickerController(
    ...
    if let pickedImage = info[.originalImage] as? UIImage {
        for n in 1...10{
            NSLog("Segmentation Started: %d", n);
            
            let operation = BlockOperation(block: { [self] in
                runSegmentation(pickedImage)
            })
            
            operation.completionBlock = {
                NSLog("Operation %d completed", n)
            }
            queue.addOperation(operation)
            //runSegmentation(pickedImage)
            NSLog("Segmentation Ended: %d", n);
        }
    }
